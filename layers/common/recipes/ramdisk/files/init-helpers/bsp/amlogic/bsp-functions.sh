: ${BSP_SHOW_OFF_COLORFUL_LIGHT_BULBS_ON_CONSOLE=true} # we are not in QEMU it's just nice to have this on the console
...
bsp_play_sound_from_file() {
	echo "$FUNCNAME():$LINENO - not implemented" ; return 0;
	if [ ! -e $BSP_AUDIO_PLAYBACK_SERVER_FIFO ] ; then
	...
	for arg in $(cat $cmdline_file); do
		case $arg in
			pscgrd.hw.audio_output_backend=*)
				export AUDIO_OUTPUT_BACKEND=${arg#*=}
				;;			
			*)
...
bsp_flasher_utils_file=/init-helpers/bsp/$bsp/bsp-flasher-utils.sh	
bsp_audio_file=/init-helpers/bsp/$bsp/bsp-audio.sh

more_files_to_source="$bsp_flasher_utils_file $bsp_audio_file"
