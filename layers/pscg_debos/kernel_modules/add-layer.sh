...
main() {
	...
	sudo chroot $ROOTFS_DIR bash -c "DEBIAN_FRONTEND=noninteractive ;  apt-get install -y $LOCAL_APT_FLAGS $pscg_debos_kmod_packages " || fatalError "Failed to run main logic"
}
